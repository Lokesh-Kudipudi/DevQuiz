...
const QuestionText = ({ text }) => {
  ...
  if (!hasCode)
    return (
      <p className="text-[var(--color-text-base)] font-medium text-sm leading-relaxed">
        {text}
      </p>
    );
  return (
    <div className="text-[var(--color-text-base)] font-medium text-sm leading-relaxed prose prose-sm max-w-none prose-headings:text-[var(--color-text-base)] prose-strong:text-[var(--color-text-base)] prose-code:text-[var(--color-text-base)] prose-a:text-[var(--color-accent)]">
      <ReactMarkdown
        rehypePlugins={[rehypeHighlight]}
        components={{
          pre({ children }) {
            return (
              <pre className="rounded-xl overflow-x-auto my-2 p-4 bg-[var(--color-surface2)] border border-[var(--color-text-base)]/[0.14] text-sm">
                {children}
              </pre>
            );
          },
...
const OAResults = () => {
    ...
    const fetch = async () => {
      try {
        const { data: res } = await axios.get(
          `/api/online-assessments/${id}/results`,
        );
        setData(res);
      } catch (err) {
        console.error(err);
  ...
  const maxPossible = data.sections.reduce(
      ...
      <div className="max-w-[1200px] mx-auto px-8 py-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-gray-400 text-sm">
                {data.sections.length} sections · {maxPossible} total marks
              </p>
            </div>
            {myParticipant && (
        ...
        {isTerminated && (
          <div className="mb-6 flex items-center gap-3 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl p-4">
            ...
            Your assessment was terminated (back navigation, internet loss, or
            manual end). Submitted sections are shown below.
          </div>
        )}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {(() => {
              ...
              const sub = mySubmissions.find(
                (ss) => ss.sectionIndex === activeSection,
              );
                  ...
                  {section.questions.map((q, qIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSkipped && (
                          <p className="pl-10 mt-2 text-xs text-gray-500">
                ...
                <div className="space-y-3">
                  {data.leaderboard.map((entry, rank) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`text-lg w-6 text-center flex-shrink-0 ${rankColors[rank] || "text-gray-500"}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`font-medium truncate ${isMe ? "text-violet-300" : "text-white"}`}
                          >
                      ...
                      <div key={ss.sectionIndex}>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="text-white font-medium">
                            {ss.score}/{section?.questionCount}
                          </span>
                        </div>
