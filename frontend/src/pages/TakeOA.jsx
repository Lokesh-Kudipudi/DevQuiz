...
const QuestionText = ({ text }) => {
  ...
  if (!hasCode)
    return <p className="text-gray-100 font-medium leading-relaxed">{text}</p>;
  return (
    <div
      className="text-[var(--color-text-base)] font-medium leading-relaxed prose prose-sm max-w-none prose-headings:text-[var(--color-text-base)] prose-strong:text-[var(--color-text-base)] prose-code:text-[var(--color-text-base)] prose-a:text-[var(--color-accent)]"
      style={{
        "--tw-prose-pre-bg": "var(--color-surface2)",
        "--tw-prose-pre-border": "var(--color-text-base)",
      }}
    >
      <ReactMarkdown
...
const TakeOA = () => {
  ...
  const [showSubmitSectionConfirm, setShowSubmitSectionConfirm] =
    useState(false);
  ...
  const terminateAssessment = useCallback(
    async (redirect = true) => {
      ...
      if (navigator.sendBeacon) {
        ...
        try {
          await axios.put(`/api/online-assessments/${id}/end`);
        } catch (_) {}
      }
      ...
      try {
        ...
        if (data.participant) {
          ...
          const nextIdx = data.sections.findIndex(
            (_, sIdx) =>
              !p.sectionSubmissions.some((ss) => ss.sectionIndex === sIdx),
          );
          if (nextIdx === -1) {
            navigate(`/oa/${id}/results`, { replace: true });
            return;
          ...
          setAnswers(
            new Array(data.sections[nextIdx].questions.length).fill(null),
          );
  ...
  const handleStartAssessment = async () => {
    try {
      ...
      setAnswers(
        new Array(oa.sections[sectionIdx].questions.length).fill(null),
      );
  ...
  const handleSubmitSection = useCallback(
    async (autoSubmit = false) => {
      ...
      try {
        ...
        const timeTaken =
          currentSection.timeLimit * 60 - (autoSubmit ? 0 : timeLeft);
        const { data } = await axios.post(
          `/api/online-assessments/${id}/submit-section`,
          {
  ...
  useEffect(() => {
    handleSubmitSectionRef.current = handleSubmitSection;
  }, [handleSubmitSection]);
        ...
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">
              {oa.sections.length} sections ·{" "}
              {oa.sections.reduce((s, sec) => s + sec.questionCount, 0)} total
              questions
            </p>
          </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="text-sm text-amber-300 space-y-1">
                ...
                <ul className="list-disc list-inside text-amber-200/80 space-y-0.5">
                  <li>
                    Once you start a section, you cannot leave until it is
                    submitted or time runs out.
                  </li>
                  <li>
                    Navigating away (back button, URL change) will{" "}
                    <strong>terminate</strong> your assessment.
                  </li>
                  <li>
                    Losing internet for 10+ seconds will terminate your
                    assessment.
                  </li>
                  <li>
                    Submitted sections are final — you cannot re-attempt them.
                  </li>
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <Card className="max-w-md w-full text-center border-violet-500/30">
            ...
            <p className="text-gray-400 mb-6">
              Once you confirm, your assessment begins.{" "}
              <span className="text-red-400 font-medium">
                Going back will terminate it permanently.
              </span>
            </p>
      ...
      <div className="min-h-screen bg-gray-950 text-white">
        ...
        {offlineCountdown !== null && (
          <div className="fixed top-0 left-0 right-0 z-50 bg-red-600 text-white text-center py-3 text-sm font-semibold">
            ⚠️ Internet connection lost. Assessment will terminate in{" "}
            {offlineCountdown}s unless you reconnect.
          </div>
        )}
          ...
          <div className="max-w-4xl mx-auto mt-2">
            <div className="h-1 bg-gray-800 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${(timeLeft / (section.timeLimit * 60)) * 100}%`,
                }}
              />
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <Card className="max-w-md w-full mx-4 border-violet-500/30 text-center">
              ...
              <p className="text-gray-400 mb-2">
                You have answered{" "}
                <span className="text-white font-semibold">{answered}</span> out
                of{" "}
              ...
              {answered < section.questions.length && (
                <p className="text-amber-400 text-sm mb-4">
                  ⚠️ {section.questions.length - answered} questions are
                  unanswered.
                </p>
              )}
              <p className="text-gray-500 text-sm mb-6">
                Once submitted, this section cannot be re-attempted.
              </p>
              <div className="flex gap-3 justify-center">
                <Button
                  variant="ghost"
        ...
        {showEndConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <Card className="max-w-md w-full mx-4 border-red-500/30 text-center">
              ...
              <p className="text-gray-400 mb-6">
                Your progress will be saved for submitted sections, but the
                current section will be{" "}
                <span className="text-red-400 font-semibold">terminated</span>{" "}
                without submission.
              </p>
              <div className="flex gap-3 justify-center">
                <Button
          ...
          {nextSection ? (
            <>
              <p className="text-gray-400 mb-6">
                ...
                ({nextSection.questionCount} questions · {nextSection.timeLimit}{" "}
                min)
              </p>
