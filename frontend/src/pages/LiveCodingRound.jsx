...
const QuestionTimer = ({ startTime, accumulatedTime, status }) => {
    ...
    return (
        <span className="font-mono font-bold text-xl text-primary-400">
            {formatTime(time)}
        </span>
    );
};
...
const LiveCodingRound = () => {
    ...
    const fetchRound = async () => {
        try {
            ...
            const { data } = await axios.get(`/api/coding-rounds/${id}?t=${new Date().getTime()}`);
            ...
            console.log('Fetched participant status:', data.participant?.questionStatus);
    ...
    const handleStartQuestion = async (questionId, url) => {
        try {
            const { data } = await axios.put(`/api/coding-rounds/${id}/questions/${questionId}/start`);
            // Open in new tab
            ...
            setParticipant(prev => {
                if (!prev) return prev;
                const newStatus = prev.questionStatus ? [...prev.questionStatus] : [];
                const index = newStatus.findIndex(qs => qs.questionId === questionId);
                if (index >= 0) {
            ...
            setError(err.response?.data?.message || 'Failed to start question');
        }
    };

    const handlePauseQuestion = async (questionId) => {
        try {
            const { data } = await axios.put(`/api/coding-rounds/${id}/questions/${questionId}/pause`);
            ...
            setError(err.response?.data?.message || 'Failed to pause question');
        }
    };
    ...
    const isCreator = round && user && (round.creator._id || round.creator) === user._id;
    ...
    const getQuestionData = (qId) => {
        if (!participant || !participant.questionStatus) return { status: 'Pending' };
        return participant.questionStatus.find(qs => qs.questionId === qId) || { status: 'Pending' };
    };
    ...
    return (
        <Layout>
            <div className="max-w-6xl mx-auto space-y-6">
                 {/* Header */}
                 <Card className="border-b-4 border-primary-600">
                    <div className="flex justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-bold text-white mb-2">{round.title}</h1>
                            <div className="flex items-center gap-4 text-gray-400">
                                <span className={timeLeft < 300 ? 'text-red-500 font-bold animate-pulse' : 'text-gray-300'}>
                        ...
                        <div className="flex items-center gap-2">
                             <Button variant="secondary" onClick={() => navigate(`/groups/${round.group._id || round.group}`)}>
                                Exit Round
                            </Button>
                            {isCreator && (
                                <Button 
                ...
                {error && <div className="bg-red-900/50 text-red-200 p-4 rounded-lg border border-red-800">{error}</div>}

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {/* Questions List */}
                    <div className="lg:col-span-2 space-y-4">
                        <h2 className="text-xl font-bold text-white mb-4">Your Challenges</h2>
                        {round.questions.map((q, idx) => {
                            const qData = getQuestionData(q._id);
                            const status = qData.status;
                            return (
                                <Card key={idx} className={`transition-all ${
                                    status === 'Passed' ? 'border-green-600/50 bg-green-900/10' : 
                                    status === 'Solving' ? 'border-yellow-600/50 bg-yellow-900/10' : ''
                                }`}>
                                    <div className="flex justify-between items-start">
                                                ...
                                                {(status === 'Solving' || status === 'Passed') && (
                                                    <div className="flex items-center gap-2 ml-4 px-3 py-1 bg-gray-800 rounded-lg">
                                                        ...
                                                        <QuestionTimer 
                                                            ...
                                                            accumulatedTime={qData.accumulatedTime || qData.timeTaken} 
                                                            status={status}
                                                        />
                                                    </div>
                                                )}
                                        ...
                                        <div className="text-right">
                                            {status === 'Pending' && (
                                                <Button onClick={() => handleStartQuestion(q._id, q.url)}>
                                                    Start Solving
                                                </Button>
                                            )}
                                            {status === 'Solving' && (
                                                    ...
                                                    <div className="flex gap-2">
                                                        {qData.lastStartTime ? (
                                                            <Button 
                                                                ...
                                                                size="sm"
                                                                className="flex-1"
                                                                onClick={() => handlePauseQuestion(q._id)}
                                                            >
                                                                Pause ⏸
                                                            </Button>
                                                        ) : (
                                                            <Button 
                                                                ...
                                                                onClick={() => handleStartQuestion(q._id, q.url)}
                                                            >
                                                                Resume ▶
                                                            </Button>
                                                        )}
